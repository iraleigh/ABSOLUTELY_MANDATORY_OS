var Processes = {
    listOfProcesses: [
      ...
      },
      {
          ...
          state: "Ready",
          main: function () {
              //Custom function: Sorts contacts list by last name.
              //Reusing data from Contact Manager:
              //Contacts from https://www.briandunning.com/sample-data/
              OS.FS.create("ContactManager_PlzSort.csv",
                "James,Butt,6649 N Blue Gum St,New Orleans,LA,70116,504-621-8927,jbutt@gmail.com\n"
              + "Josephine,Darakjy,4 B Blue Ridge Blvd,Brighton,MI,48116,810-292-9388,josephine_darakjy@darakjy.org\n"
              + "Art,Venere,8 W Cerritos Ave #54,Bridgeport,NJ,8014,856-636-8749,art@venere.org\n"
              + "Lenna,Paprocki,639 Main St,Anchorage,AK,99501,907-385-4412,lpaprocki@hotmail.com\n"
              ...
              var oContactManagerFile = OS.FS.open("ContactManager_PlzSort.csv");
              var length = OS.FS.length(oContactManagerFile);
              var content = "";
              ...
              OS.FS.close("ContactManager_PlzSort.csv");

              //Sorts by the second element by using Stack Overflow magic
              rows.sort(function (a, b) { return a[1].localeCompare(b[1]); });

              //Drop contents into a string to be written to file
              var output = "";
              for (i = 0; i < rows.length; i++) {
                  for (j = 0; j < rows[i].length; j++) {
                      output += rows[i][j];
